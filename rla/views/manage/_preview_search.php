...
$this->registerJS("$(function(){
    $('#jstree_container').on('select_node.jstree', function (e, data) {
        if(!data.node.id.startsWith('j')){
            window.location = '" . Url::to(['preview']) . "' + '?searchModel=' + data.node.id;
            showNotification('لطفا تا بارگذاری صفحه صبر کنید...', 'info');
        }else{
            showNotification('گزینه انتخاب شده از نوع داده گاه نیست!', 'error');
        }
    });
